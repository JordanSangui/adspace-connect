...
import { formatXAF, formatDate } from "@/lib/mock-data";
import {
  useAppStore,
  selectAgencyOffers,
  selectAgencyContracts,
  selectPublishedVenues,
  selectAgencyTotalSpent,
...
export default function AgencyDashboard() {
  ...
  const activeContracts = agencyContracts.filter(
    (c) => c.status === "En cours" || c.status === "Contrat signé" || c.status === "Paiement"
  );
  const recommendedVenues = publishedVenues.slice(0, 3);
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 stagger-children">
        <StatsCard
          ...
          value={String(agencyOffers.length)}
          icon={
        ...
        <StatsCard
          ...
          trend={agencyOffers.length > 0
            ? `${Math.round((acceptedOffers.length / agencyOffers.length) * 100)}% taux`
            : undefined}
          trendUp={true}
        />
        <StatsCard
